...
(define (parse-item n l)
  (let ((padding (node-get-data n 'padding)))
    (cond ((and (blank-node? (last-child n)) (empty-line? l))
           (close-node (remove-last-child n)))
          ((empty-line? l)
           (if (open-descendant? n 'fenced-code)
               (parse-container-block n l)
               (add-child-node (replace-last-child n (close-node (last-child n)))
                               (make-blank-node))))
          ((n-spaces? padding l)
           (parse-container-block (if (blank-node? (last-child n))
                                      (remove-last-child n)
                                      n)
                                  (substring l padding)))
...
(define (make-item line offset spaces)
  (let ((padding (string-length spaces)))
    (cond ((>= padding 5)
          ...
          ((< padding 1)
           (make-item-node (make-blank-node) (+ offset 1)))
...
(define (parse-clean-up n col)
  (cond ((not (node? n)) (col n '()))
        ...
        ((item-node? n) (remove-blank-node n col))
...
(define (remove-blank-node n col)
  (cond ((no-children? n) (col n '()))
        ((blank-node? (last-child n))
         (col (remove-last-child n) '()))
        (else (map&co parse-clean-up (node-children n)
                      (lambda (v d)
                        (col (make-node (node-type n)
                                        (node-data n)
