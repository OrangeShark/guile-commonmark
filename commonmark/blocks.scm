...
(define (parse-item n l)
  ...
  (define (prev-code-block? node)
    (and (> (length (node-children node)) 1) (code-block-node? (cadr (node-children node)))))
          ...
          ((empty-line? l)
           (cond ((open-descendant? n 'fenced-code)
                 ...
                 (else (add-child-node (replace-last-child n (close-node (last-child n)))
                                       (make-blank-node)))))
          ((n-spaces? padding l)
           (parse-container-block (if (and (blank-last-child? n) (prev-code-block? n))
                                      (remove-last-child n)
                                      n)
                                  (substring l padding)))
