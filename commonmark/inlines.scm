...
(define (parse-emphasis text nodes delim-stack)
            ...
            (match (match-delim matching-delim delim)
              ...
              ((od #f)
               (parse-char (text-advance text (delimiter-count delim))
                           ...
                           (delim-stack-replace-delim ds od)))
              ((#f cd)
    ...
    (cond ((and (delimiter-close? delim) (delimiter-open? delim))
           (let ((matching-delim (matching-opening? delim-stack delim)))
             (if matching-delim
                 ...
                 (parse-char (text-advance text (delimiter-count delim))
                             '()
                             (delim-stack-push delim-stack delim nodes)))))
...
(define (parse-newline text nodes delim-stack)
...
(define (parse-backslash text nodes delim-stack)
...
(define (parse-code-span text nodes delim-stack)
...
(define (parse-char text nodes delim-stack)
  (if (text-end? text)
      (pop-remaining-delim nodes delim-stack)
      (case (text-char text)
        ((#\newline) (parse-newline text nodes delim-stack))
...
(define (parse-inline node)
  (let ((text (last-child (last-child node))))
    (make-node (node-type node) (node-data node) (parse-char (make-text text 0) '() (make-empty-delim-stack)))))
